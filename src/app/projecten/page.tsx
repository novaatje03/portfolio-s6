
export default function Projecten() {

    return (
    <>
       <div id="ProjectenOverzicht">
        <div id="Overzicht">
      <div id="Kopje"><h1>Groepsprojecten</h1></div>
      <div id="Projecten">
      <div id="MasterMind">
        <a href="/mastermindproject"><img id="MasterMindFoto" src="/mastermindsfoto.png"/></a>
        <h1>Master Mind</h1>
      </div>
      <div id="VerzuimNavigator">
        <a href="/verzuimnavigatorproject"><img id="VerzuimNavigatorFoto" src="/verzuimnavigatorfoto.png"/></a>
        <h1>Verzuimnavigator</h1>
      </div>
      </div>
      </div>
      </div>
      <div id="ProjectOverzicht">
        <div id="Project2">
          <div id="Kopje"><h1>Persoonlijk Project</h1></div>
          <div id="PersoonlijkProject">
            <a href="/personalproject"><img id="PersoonlijkProjectFoto" src="/aliexpress.png"/></a>
          </div>
        </div>
      </div>
    </>
    )
    }